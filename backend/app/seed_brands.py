"""경쟁사 브랜드 시드(B-2) — 주요 가전 브랜드 별칭 등록.

매처가 brand.aliases_json 을 순회하므로, 등록 시 brand_raw='삼성전자' 등이
자동으로 brand_id에 매핑된다(is_own=False). 이를 통해 브랜드별 가격/변동 비교 가능.
실행:  python -m app.seed_brands
"""
from sqlalchemy import select

from app.database import Base, SessionLocal, engine
from app.models import Brand

# (표준명, [별칭...]) — 자사(쿠쿠) 제외 경쟁/주요 브랜드
COMPETITORS: list[tuple[str, list[str]]] = [
    # 삼성/LG는 네이버 brand_raw에 라인명(비스포크·디오스 등)이 들어오므로 별칭으로 흡수
    ("삼성전자", ["삼성", "SAMSUNG", "삼성전자", "비스포크", "BESPOKE", "그랑데", "셰프컬렉션"]),
    ("LG전자", [
        "LG", "엘지", "LG전자", "LG ELECTRONICS",
        "디오스", "DIOS", "트롬", "TROMM", "오브제", "오브제컬렉션", "OBJET",
        "스타일러", "STYLER", "휘센", "WHISEN", "통돌이", "코드제로", "퓨리케어", "틔운",
        "프라엘", "PRAEL",  # LG 뷰티 디바이스
    ]),
    ("위니아", ["위니아", "WINIA", "딤채", "위니아딤채"]),
    ("쿠첸", ["쿠첸", "CUCHEN"]),
    ("코웨이", ["코웨이", "COWAY"]),
    ("SK매직", ["SK매직", "SK MAGIC", "에스케이매직"]),
    ("청호나이스", ["청호나이스", "청호"]),
    ("위닉스", ["위닉스", "WINIX"]),
    ("샤오미", ["샤오미", "XIAOMI"]),
    ("다이슨", ["다이슨", "DYSON"]),
    ("필립스", ["필립스", "PHILIPS"]),
    ("드롱기", ["드롱기", "DELONGHI"]),
    ("발뮤다", ["발뮤다", "BALMUDA"]),
    ("브라운", ["브라운", "BRAUN"]),
    ("일렉트로룩스", ["일렉트로룩스", "ELECTROLUX"]),
    ("캐리어", ["캐리어", "CARRIER"]),
    ("신일", ["신일", "SHINIL", "신일전자"]),
    ("한일", ["한일", "한일전기"]),
    ("휴롬", ["휴롬", "HUROM"]),
    ("바디프랜드", ["바디프랜드", "BODYFRIEND"]),
    ("코지마", ["코지마", "KOJIMA"]),
    ("세라젬", ["세라젬", "CERAGEM"]),
    ("쿠진아트", ["쿠진아트", "CUISINART"]),
    ("키친아트", ["키친아트", "KITCHENART"]),
    # 2차 보강 — 미매칭(기타/미상) 상위 실브랜드
    ("미닉스", ["미닉스", "MINIX"]),
    ("닌자", ["닌자", "NINJA"]),
    ("로보락", ["로보락", "ROBOROCK"]),
    ("브리타", ["브리타", "BRITA"]),
    ("나비엔", ["나비엔", "NAVIEN", "나비엔매직"]),
    ("이누스", ["이누스", "INUS"]),
    ("한경희", ["한경희", "한경희생활과학"]),
    ("에브리봇", ["에브리봇", "EVERYBOT"]),
    ("매직쉐프", ["매직쉐프", "MAGIC CHEF", "MAGICCHEF"]),
    ("린나이", ["린나이", "RINNAI"]),
    ("테팔", ["테팔", "TEFAL"]),
    ("네스프레소", ["네스프레소", "NESPRESSO"]),
    ("드리미", ["드리미", "DREAME"]),
    ("파나소닉", ["파나소닉", "PANASONIC"]),
    ("파세코", ["파세코", "PASECO"]),
    ("콘에어", ["콘에어", "CONAIR"]),
    ("아이닉", ["아이닉", "AINIC"]),
    ("보국전자", ["보국전자", "보국"]),
    ("카스", ["카스", "CAS"]),
    ("쿠잉", ["쿠잉", "CUING", "쿠잉전자"]),
    # 3차 보강 — 인지도 높은 미매칭 브랜드
    ("샤크", ["샤크", "SHARK"]),
    ("오아", ["오아"]),
    ("노비타", ["노비타", "NOVITA"]),
    ("인바디", ["인바디", "INBODY"]),
    ("에코백스", ["에코백스", "ECOVACS"]),
    ("브레빌", ["브레빌", "BREVILLE"]),
    ("보스", ["BOSE", "보스"]),
    ("유닉스", ["유닉스", "UNIX"]),
    ("엡손", ["엡손", "EPSON"]),
    ("뷰소닉", ["뷰소닉", "VIEWSONIC"]),
    ("엑스지미", ["엑스지미", "XGIMI"]),
    ("루메나", ["루메나", "LUMENA"]),
    ("JMW", ["JMW"]),
    ("보만", ["보만", "BOMANN"]),
    ("듀플렉스", ["듀플렉스", "DUPLEX"]),
    ("JBL", ["JBL"]),
    ("대림바스", ["대림바스", "대림"]),
    # 4차 보강 — 기타/미상 상위 실브랜드(제목 첫 토큰 매칭으로 회복)
    ("조지루시", ["조지루시", "ZOJIRUSHI"]),
    ("피코그램", ["피코그램", "PICOGRAM"]),
    ("하츠", ["하츠", "HAATZ"]),
    ("블라우풍트", ["블라우풍트", "BLAUPUNKT"]),
    ("프롬비", ["프롬비", "FROMB"]),
    ("무아스", ["무아스", "MUAS"]),
    ("엘리카", ["엘리카", "ELICA"]),
    ("제로웰", ["제로웰", "ZEROWEL"]),
    ("재원전자", ["재원전자", "재원"]),
    ("롯데알미늄", ["롯데알미늄"]),
    ("한솔", ["한솔"]),
    ("콕스타", ["콕스타", "COXTA"]),
    ("앳플리", ["앳플리"]),
    ("단미", ["단미"]),
    ("모바", ["모바", "MOBA"]),
    ("에어메이드", ["에어메이드", "AIRMADE"]),
    ("가이타이너", ["가이타이너"]),
    ("브람스", ["브람스", "BRAHMS"]),
    ("자일렉", ["자일렉"]),
    ("엠앤에스코리아", ["엠앤에스코리아"]),
    ("우성", ["우성", "우성전자"]),
    # 5차 보강 — 신규 시장 카테고리(음향·건강측정·미용·청소 등) 미시드 실브랜드
    ("오므론", ["오므론", "OMRON"]),
    ("카처", ["카처", "KARCHER", "캐처"]),
    ("소니", ["소니", "SONY"]),
    ("마샬", ["마샬", "MARSHALL"]),
    ("브리츠", ["브리츠", "BRITZ"]),
    ("오랄비", ["오랄비", "오랄-비", "ORAL-B", "ORALB"]),
    ("워터픽", ["워터픽", "WATERPIK"]),
    ("아쿠아픽", ["아쿠아픽", "AQUAPICK"]),
    ("유라이크", ["유라이크", "ULIKE"]),
    ("풀리오", ["풀리오", "PULIO"]),
    ("델키", ["델키", "DELKI"]),
    ("리큅", ["리큅", "LEQUIP"]),
    ("휴렉", ["휴렉", "HUREC"]),
    ("엔유씨", ["엔유씨", "엔유씨전자", "NUC"]),
    ("비쎌", ["비쎌", "비셀", "BISSELL"]),
    ("블랙앤데커", ["블랙앤데커", "블랙앤덱커", "BLACK&DECKER", "BLACKDECKER"]),
    ("하이얼", ["하이얼", "HAIER"]),
    ("일월", ["일월", "일월매트"]),
    ("대성마리프", ["대성마리프", "마리프"]),
    ("한일메디칼", ["한일메디칼", "한일메디컬"]),
    ("휴비딕", ["휴비딕", "HUBDIC"]),
    ("녹십자", ["녹십자", "녹십자MS", "녹십자엠에스"]),
    ("케어센스", ["케어센스", "케어센스N", "CARESENS"]),
    ("아이센스", ["아이센스", "ISENS"]),
    ("카리스", ["카리스", "CARIS"]),
    ("대원", ["대원", "대원냉열"]),
    ("한샘", ["한샘", "HANSSEM"]),
    ("딜라이트소다", ["딜라이트소다", "딜라이트"]),
    # 6차 보강 — 기타/미상 중 '인지도 높은' 단일카테고리 실브랜드(데이터 회복 검증 완료)
    ("도루코", ["도루코", "DORCO"]),
    ("레이캅", ["레이캅", "RAYCOP"]),
    ("쿠빙스", ["쿠빙스", "KUVINGS"]),
    ("디베아", ["디베아", "DIBEA"]),
    ("메디큐브", ["메디큐브", "MEDICUBE"]),
    ("에이피알", ["에이피알", "APR"]),
    ("밀레", ["밀레", "MIELE"]),
    ("TCL", ["TCL"]),
    ("애플", ["애플", "APPLE", "에어팟", "AIRPODS"]),
    ("바워스앤윌킨스", ["바워스앤윌킨스", "BOWERS", "BOWERS&WILKINS"]),
    ("젠하이저", ["젠하이저", "SENNHEISER"]),
    ("앤커", ["앤커", "ANKER", "사운드코어", "SOUNDCORE"]),
    ("아이리버", ["아이리버", "IRIVER"]),
    ("풀무원", ["풀무원건강생활", "풀무원"]),
    ("iSi", ["iSi", "ISI"]),
    ("아르케", ["아르케", "ARKE"]),
    ("넥스코", ["넥스코", "NEXCO"]),
    ("라익미", ["라익미", "LIKEME"]),
    ("리하스", ["리하스", "RIHAS"]),
    ("대흥소프트밀", ["대흥소프트밀"]),
    ("라진", ["라진"]),
    ("싱크루션", ["싱크루션"]),
    ("돌부심", ["돌부심"]),
    ("해마루", ["해마루"]),
    ("ELBA", ["ELBA", "엘바"]),
    ("뷰앤디", ["뷰앤디"]),
    ("스킨사이언스", ["스킨사이언스"]),
    ("고릴라왁싱", ["고릴라왁싱"]),
    # 7차 보강 — 다나와 카테고리별 브랜드 필터 교차검증(우리 데이터에 존재하는 실브랜드)
    # 커피/주방
    ("스메그", ["스메그", "SMEG"]),
    ("유라", ["유라", "JURA"]),
    ("돌체구스토", ["돌체구스토", "DOLCE GUSTO", "네스카페돌체구스토"]),
    ("바이타믹스", ["바이타믹스", "VITAMIX"]),
    ("동서식품", ["동서식품", "카누", "KANU"]),
    ("오스카전자", ["오스카전자", "오스카", "OSCAR"]),
    ("풍년", ["풍년", "PN풍년"]),
    ("마이어", ["마이어", "MAYER"]),
    ("도무스", ["도무스", "DOMUS"]),
    ("지티스", ["지티스"]),
    ("까사비노", ["까사비노"]),
    # 영상·음향
    ("옵토마", ["옵토마", "OPTOMA"]),
    ("ETOE", ["ETOE"]),
    ("제네바", ["제네바", "GENEVA"]),
    # 건강측정
    ("덱스콤", ["덱스콤", "DEXCOM"]),
    ("로슈", ["로슈", "ROCHE", "아큐첵", "ACCU-CHEK"]),
    ("애보트", ["프리스타일", "FREESTYLE", "애보트", "ABBOTT"]),
    ("닥터웰", ["닥터웰", "DR.WELL", "DRWELL"]),
    ("동국제약", ["동국제약"]),
    # 미용
    ("셀리턴", ["셀리턴", "CELLRETURN"]),
    ("실큰", ["실큰", "SILKN", "SILK'N"]),
    ("포레오", ["포레오", "FOREO"]),
    ("센텔리안24", ["센텔리안24", "센텔리안"]),
    ("비타그램", ["비타그램", "VITAGRAM"]),
    ("모즈", ["모즈스웨덴", "모즈", "MOZ"]),
    # 마사지·안마
    ("제스파", ["제스파", "ZESPA"]),
    ("휴테크", ["휴테크", "휴테크산업"]),
    ("닥터에어", ["닥터에어", "DR.AIR"]),
    ("누하스", ["누하스", "NUHAS"]),
    ("마리아띠", ["마리아띠"]),
    # 생활·환경
    ("에어렉스", ["에어렉스"]),
    ("캐로스", ["캐로스", "CAROS"]),
    ("홈스미스", ["홈스미스", "HOMSMITH"]),
    ("동양이지텍", ["동양이지텍", "이지텍"]),
    ("유원트", ["유원트", "UWANT"]),
    ("레벤트", ["레벤트", "LEVENT"]),
]


def seed_brands() -> int:
    Base.metadata.create_all(bind=engine)
    db = SessionLocal()
    added = 0
    updated = 0
    try:
        for name, aliases in COMPETITORS:
            b = db.scalar(select(Brand).where(Brand.name == name))
            if not b:
                db.add(Brand(name=name, is_own=False, aliases_json=aliases))
                added += 1
            elif (b.aliases_json or []) != aliases:
                b.aliases_json = aliases  # 별칭 변경 시 갱신(라인명 추가 등)
                updated += 1
        db.commit()
        print(f"[seed_brands] 경쟁 브랜드 {added}개 추가 / {updated}개 별칭 갱신")
        return added
    finally:
        db.close()


if __name__ == "__main__":
    seed_brands()
